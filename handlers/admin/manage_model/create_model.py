...
from keyboards.inline.cancel_state import cancel
...
async def model_add(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Start create a new model.\n"
                              "Come up with a new model name", reply_markup=cancel())
...
async def get_name_model(msg: Message, state: FSMContext):
    await msg.answer(f"Model name is \r\n{msg.text}\r\n"
                     "Now add description for model", reply_markup=cancel())
...
async def get_description_model(msg: Message, state: FSMContext):
    await msg.answer(f"Description of model is \r\n{msg.text}\r\n"
                     "Now add photo for model", reply_markup=cancel())
...
async def get_photo_model(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"Photo is a success save.\n\n"
                     "Now add link for download file of model", reply_markup=cancel())
...
async def cancel_state(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text("Cancel operation", reply_markup=model_keyboard_tools())


@router.callback_query(F.data == "back_state")
async def back_state(call: CallbackQuery, state: FSMContext):
    current_state = await state.get_state()
    if current_state == "ModelAdd:get_name":
        await state.set_data({})
        await state.clear()
        await call.message.edit_text("Back to category", reply_markup=model_keyboard_tools())
    if current_state == "ModelAdd:get_description":
        await state.set_state(ModelAdd.get_name)
        data = await state.get_data()
        await call.message.edit_text(f"Edit name {data['get_name']}", reply_markup=cancel())
    if current_state == "ModelAdd:get_photo":
        await state.set_state(ModelAdd.get_description)
        data = await state.get_data()
        await call.message.edit_text(f"Edit description {data['get_description']}", reply_markup=cancel())
    if current_state == "ModelAdd:get_link_file":
        await state.set_state(ModelAdd.get_photo)
        data = await state.get_data()
        await call.message.edit_text(f"Edit old photo with id: {data['get_photo']}", reply_markup=cancel())
    else:
        return
